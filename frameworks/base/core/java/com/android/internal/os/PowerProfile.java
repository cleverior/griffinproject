        ...
        try {
            ...
            while (true) {
                ...
                if (parsingArray && !element.equals(TAG_ARRAYITEM)) {
                    // Finish array
                    //NOTE: Bug 612553 The Exception happen when click battery center BEG
                    // add by sprd start
                    // make a judgement before putting an array into sPowerMap
                    if(array.size() != 0){
                        sPowerMap.put(arrayName, array.toArray(new Double[array.size()]));
                    }
                    //<-- The Exception happen when click battery center END
                    parsingArray = false;
                }
