...
public final class UiAutomation {
    ...
    public void waitForIdle(long idleTimeoutMillis, long globalTimeoutMillis)
        ...
        /*
            This modification is for bug 616522 that is a cts failure.
            You can find out more details about this bug on bugzilla.
            Becuase there is no regular solution for this bug,
            we have to use this temporary workaroud to fix the cts failure.
            if google fixes this issue on next cts version, we SHALL REMOVE this modification.
        */
        try {
            //Log.d("HSUAN", "waitForIdle workaround");
            StackTraceElement[] steArray = new Throwable().getStackTrace();
            if (steArray.length > 4) {
                if ("com.android.cts.encryptionapp.EncryptionAppTest".equals(steArray[4].getClassName())
                        && "dismissKeyguard".equals(steArray[4].getMethodName())) {
                    Log.w(LOG_TAG, "waitForIdle workaroud for cts, should be removed if google's bug is fixed");
                    SystemClock.sleep(6000);
                }
            }
            //Log.d("HSUAN", "waitForIdle workaround end");
        } catch (Exception e) {
