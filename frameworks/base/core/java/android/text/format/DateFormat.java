...
public class DateFormat {
	...
	public static java.text.DateFormat getDateFormatEx(Context context) {
        String value = Settings.System.getString(context.getContentResolver(),
                Settings.System.DATE_FORMAT);
            ...
            if (week >= 0 && month >= 0 && day >= 0 && year >= 0) {
                ...
                if (week < day) {
                    if (year < month && year < day) {
                        if (month < day) {
                            template = context.getString(com.android.internal.R.string.wday_year_month_day);
                            result = String.format(template, weekValue, yearValue, monthValue, dayValue);
                        } else {
                            template = context.getString(com.android.internal.R.string.wday_year_day_month);
                            result = String.format(template, weekValue, yearValue, dayValue, monthValue);
                        }
                    } else if (month < day) {
                        if (day < year) {
                            template = context.getString(com.android.internal.R.string.wday_month_day_year);
                            result = String.format(template, weekValue, monthValue, dayValue, yearValue);
                        } else {
                            template = context.getString(com.android.internal.R.string.wday_month_year_day);
                            result = String.format(template, weekValue, monthValue, yearValue, dayValue);
                        }
                    } else {
                        if (month < year) {
                            template = context.getString(com.android.internal.R.string.wday_day_month_year);
                            result = String.format(template, weekValue, dayValue, monthValue, yearValue);
                        } else {
                            template = context.getString(com.android.internal.R.string.wday_day_year_month);
                    ...
                    if (year < month && year < day) {
                        if (month < day) {
                            template = context.getString(com.android.internal.R.string.year_month_day_wday);
                            result = String.format(template, yearValue, monthValue, dayValue, weekValue);
                        } else {
                            template = context.getString(com.android.internal.R.string.year_day_month_wday);
                            result = String.format(template, yearValue, dayValue, monthValue, weekValue);
                        }
                    } else if (month < day) {
                        if (day < year) {
                            template = context.getString(com.android.internal.R.string.month_day_year_wday);
                            result = String.format(template, monthValue, dayValue, yearValue, weekValue);
                        } else {
                            template = context.getString(com.android.internal.R.string.month_year_day_wday);
                            result = String.format(template, monthValue, yearValue, dayValue, weekValue);
                        }
                    } else {
                        if (month < year) {
                            template = context.getString(com.android.internal.R.string.day_month_year_wday);
                            result = String.format(template, dayValue, monthValue, yearValue, weekValue);
                        } else {
                            template = context.getString(com.android.internal.R.string.day_year_month_wday);
