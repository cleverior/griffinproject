...
public class DatabaseUtils {
    ...
    /** SPRD:Bug 613852  diff language,same key @{ */
    public static String getCollationKeyWithDefaultLocal(String name) {
        byte [] arr = getCollationKeyInBytesWithDefaultLocal(name);
        try {
            return new String(arr, 0, getKeyLen(arr), "ISO8859_1");
        } catch (Exception ex) {
            return "";
    ...
    /** SPRD:Bug 613852  diff language,same key @{ */
    private static byte[] getCollationKeyInBytesWithDefaultLocal(String name) {
        if (mCollAudio == null) {
            mCollAudio = Collator.getInstance(Locale.ENGLISH);
            mCollAudio.setStrength(Collator.PRIMARY);
        }
        return mCollAudio.getCollationKey(name).toByteArray();
    }

    private static Collator mCollAudio = null;
    /** @} */
    /**
