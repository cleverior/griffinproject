...
public class AutoCompleteTextView extends EditText implements Filter.FilterListener {
    ...
    private void updateDropDownForFilter(int count) {
        ...
        /*
         * SPRD: modify 20161017 Spreadtrum of 607199 Add WeakReference
         * reference type, access to the current mObserver.ViewReference to see
         * whether the current view was GC recycling, if not, from the new added
         * to the ListAdaptrer, show the drop down if present on screen. @{
         */
        WeakReference<AutoCompleteTextView> adapter = mObserver.mViewReference;
        final AutoCompleteTextView textView = adapter.get();
        ListAdapter viewAdapter = null;
        if (textView != null) {
            viewAdapter = textView.mAdapter;
        }
        if (viewAdapter != null) {
            if ((viewAdapter.getCount() > 0 || dropDownAlwaysVisible)&& enoughToFilter) {
        ...
        /*@}*/
        }else if ((count > 0 || dropDownAlwaysVisible) && enoughToFilter) {
            if (hasFocus() && hasWindowFocus() && mPopupCanBeUpdated) {
                showDropDown();
            }
        } else if (!dropDownAlwaysVisible && isPopupShowing()) {
