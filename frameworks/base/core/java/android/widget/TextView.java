        ...
        switch (getTextAlignment()) {
            case TEXT_ALIGNMENT_GRAVITY:
                switch (mGravity & Gravity.RELATIVE_HORIZONTAL_GRAVITY_MASK) {
                    case Gravity.START:
                        //modify by bug#414869
                        alignment = (getLayoutDirection() == LAYOUT_DIRECTION_RTL) ?
                        Layout.Alignment.ALIGN_RIGHT : Layout.Alignment.ALIGN_NORMAL;
                        break;
                    case Gravity.END:
                        //modify by bug#414869
                        alignment = (getLayoutDirection() == LAYOUT_DIRECTION_RTL) ?
                        Layout.Alignment.ALIGN_LEFT : Layout.Alignment.ALIGN_OPPOSITE;
                        break;
                    case Gravity.LEFT:
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        final boolean touchIsFinished = (action == MotionEvent.ACTION_UP) &&
         ...
         if ((mMovement != null || onCheckIsTextEditor()) && isEnabled()
            ...
            if (mMovement != null) {
                /* SPRD: Bug  607513 - with WEB ,long click textview, second time will happen crash. @{ */
