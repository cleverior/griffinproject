...
public class Editor {
    ...
    private void replaceWithSuggestion(@NonNull final SuggestionInfo suggestionInfo) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (suggestionSpansStarts[i] <= spanStart && suggestionSpansEnds[i] >= spanEnd) {
                /* SPRD: SPRD: modify 20160902 Spreadtrum of 594494 add check for span range @{ */
                mTextView.setSpan_internal(suggestionSpans[i], suggestionSpansStarts[i],
                     Math.min(suggestionSpansEnds[i] + lengthDelta, mTextView.length()),
                        suggestionSpansFlags[i]);
            }
        }
        // Move cursor at the end of the replaced word
        final int newCursorPosition = Math.min(spanEnd + lengthDelta, mTextView.length());
        /* @} */
    ...
    public class SuggestionsPopupWindow extends PinnedPopupWindow implements OnItemClickListener {
        ...
        private boolean updateSuggestions() {
            ...
            /* SPRD: Bug  609107 - setSelection will happen crash. @{ */
            android.util.Log.d(TAG,"setSpan spanUnionStart: "+spanUnionStart+", spanUnionEnd: "+spanUnionEnd+", Text.len: "+(mTextView.getText().length())+", Text: "+String.valueOf(mTextView.getText()));
            /* @} */
