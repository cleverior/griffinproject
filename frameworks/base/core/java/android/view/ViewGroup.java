...
public abstract class ViewGroup extends View implements ViewParent, ViewManager {
    ...
    private static boolean resetCancelNextUpFlag(@NonNull View view) {
        /* SPRD: Bug 652695 - NonNull is unuseful, should check whether view is null or not @{ */
        if(view == null) {
            Log.w(TAG,"This child is null, so we consider it's disappeared (Monkey test ?). Then we need cancel this touchEvent. ");
            return true;
        } else if((view.mPrivateFlags & PFLAG_CANCEL_NEXT_UP_EVENT) != 0) {
