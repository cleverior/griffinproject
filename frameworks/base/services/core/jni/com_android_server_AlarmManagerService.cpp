...
int AlarmImplAlarmDriver::setTime(struct timeval *tv)
{
    ...
    if (res < 0) {
        ...
        ALOGD("AlarmImplAlarmDriver::setTime, drm_ntp, _delta: %lld, delta_alarm: %lld, tv->tv_sec: %ld, old_time.tv_sec: %ld.", (long long)_delta, (long long)delta_alarm, tv->tv_sec, old_time.tv_sec);
    }
