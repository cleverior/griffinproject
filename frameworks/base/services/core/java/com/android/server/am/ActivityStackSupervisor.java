...
public final class ActivityStackSupervisor implements DisplayListener {
    ...
    void moveTasksToFullscreenStackLocked(int fromStackId, boolean onTop) {
        ...
        try {
            ...
            // Google: We are moving all tasks from the docked stack to the fullscreen stack,
            // which is dismissing the docked stack, so resize all other stacks to
            // fullscreen here already so we don't end up with resize trashing.
            // SPRD: bug647241, above workflow may create new stack while deocked stack is exists.
            // Docked stack exists means that WMS attachStack will return non-null bounds to make
            // the new created stack non-fullscreen, so we need to resize it after the move action
            // to ensure that all other stacks will be resized to fullscreen.
            if (fromStackId == DOCKED_STACK_ID)  {
