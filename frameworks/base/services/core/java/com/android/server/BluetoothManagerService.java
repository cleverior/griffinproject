...
class BluetoothManagerService extends IBluetoothManager.Stub {
    ...
    /** @hide*/
    public boolean isBleFeatureSupport() {
        PackageManager pm = mContext.getPackageManager();
        if (pm.hasSystemFeature(PackageManager.FEATURE_BLUETOOTH_LE) == false) {
           Slog.d(TAG, "Bluetooth ble is not support");
           return false;
        }
        return true;
    }
        ...
        if (isBleAppPresent() == false) {
            ...
            try {
                if (mBluetoothGatt != null) {
                    Slog.d(TAG, "Before do unregAll operation");
                    mBluetoothGatt.unregAll();
                } else {
                    Slog.d(TAG, "BLE not support, do onBrEdrDown operation");
                    if (mBluetooth != null) mBluetooth.onBrEdrDown();
                }
            } catch (RemoteException e) {
