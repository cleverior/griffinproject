...
final class InputMonitor implements InputManagerService.WindowManagerCallbacks {
    ...
    public InputMonitor(WindowManagerService service) {
        mService = service;
        mWMSLog = WMSFactory.getLogInstance();
    }
        ...
        synchronized (mService.mWindowMap) {
            ...
            if (windowState != null) {
                ...
                Slog.i(TAG_WM, "Input event dispatching timed out "
                        ...
                        + ".  Reason: " + reason);
                mWMSLog.dumpWindowStatus(appWindowToken);
            } else {
                Slog.i(TAG_WM, "Input event dispatching timed out "
                        + ".  Reason: " + reason);
            }

            final int numDisp = mService.mDisplayContents.size();
            for (int displayNdx = 0; displayNdx < numDisp; ++displayNdx) {
                final DisplayContent displayContent = mService.mDisplayContents.valueAt(displayNdx);
                Slog.i(TAG_WM, "Dump Display#" + displayNdx + " windows:");
                mWMSLog.dumpWindowStatus(displayContent);
            }
