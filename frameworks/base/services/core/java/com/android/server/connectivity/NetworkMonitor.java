...
import static android.net.NetworkCapabilities.NET_CAPABILITY_OPEN;
...
public class NetworkMonitor extends StateMachine {
    ...
    //web autoauth
    private static final int WIFI_WEB_AUTOAUTH =
             SystemProperties.getInt("persist.sys.autoauth.enable", 0);
    private static final String TAG = NetworkMonitor.class.getSimpleName();
    private static final String DEFAULT_SERVER = "connectivitycheck.gstatic.com";
    //SPRD: Bug#488291 Add for ap in china can't pass through Google network detection server Feature BEG-->
    //private static final int SOCKET_TIMEOUT_MS = 10000;
    private static final int SOCKET_TIMEOUT_MS = 2000; // android original 10000
    //<-- Add For ap in china can't pass through Google network detection server Feature END
    ...
    private static final int MAX_REEVALUATE_COUNTS_FOR_OPENAP = 3;
            ...
            switch (message.what) {
                case CMD_REEVALUATE:
                    ...
                    if (probeResult.isSuccessful()) {
                        ...
                        if (ConnectivityManager.TYPE_WIFI == mNetworkAgentInfo.networkInfo.getType()) {
                            //Although isPortal,we still inform ConnectivityService that wifi is validated pass.
                            //So that mobile data network will be disconnected, and authentication page can
                            //be redirected to.
                            validationLog("EvaluatingState: send RESULT_VALID for wifi");
                            mConnectivityServiceHandler.sendMessage(obtainMessage(EVENT_NETWORK_TESTED,
                                NETWORK_TEST_RESULT_VALID, mNetId, probeResult.mRedirectUrl));
                        } else {
                            mConnectivityServiceHandler.sendMessage(obtainMessage(EVENT_NETWORK_TESTED,
    ...
    protected CaptivePortalProbeResult sendHttpProbe(URL url, int probeType) {
        ...
        try {
            ...
            if (ConnectivityManager.TYPE_WIFI == mNetworkAgentInfo.networkInfo.getType()) {
                validationLog("sendHttpProbe: mAttempts = " + mAttempts);
                validationLog("sendHttpProbe: networkCapabilities = " + mNetworkAgentInfo.networkCapabilities);

                if(mNetworkAgentInfo.networkCapabilities.hasCapability(NET_CAPABILITY_OPEN) &&
                    mAttempts <= MAX_REEVALUATE_COUNTS_FOR_OPENAP){
                    //It is considered that login may be needed when receives exception via wifi
                    //connected to an open AP,so set httpResponseCode to 599 and validation will
                    //be retried.
                    validationLog("sendHttpProbe: set httpResponseCode to 599 for Open AP");
    ...
    //=============================================================================
    //SPRD: Bug#488291 Add for ap in china can't pass through Google network detection server Feature BEG-->
    private static final boolean RESET_DEFAULT_HTTP_RESPONSE =
            SystemProperties.getBoolean("reset_default_http_response", true);
    //<-- Add For ap in china can't pass through Google network detection server Feature END
}
