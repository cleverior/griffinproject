...
import com.android.server.power.PMSFactory;
import com.android.server.power.AbsPowerManagerServiceUtils;
...
public final class PowerManagerService extends SystemService
    ...
    public void onBootPhase(int phase) {
        synchronized (mLock) {
            if (phase == PHASE_THIRD_PARTY_APPS_CAN_START) {
                ...
                PMSFactory.getInstance().createPowerManagerServiceUtils(mContext).setBootCompleted(true);
    ...
    public void systemReady(IAppOpsService appOps) {
        synchronized (mLock) {
            ...
            PMSFactory.getInstance().initExtraPowerManagerService(mContext);
            ...
            resolver.registerContentObserver(Settings.System.getUriFor(
                    ...
                    false, mSettingsObserver, UserHandle.USER_ALL);
            PMSFactory.getInstance().createPowerManagerServiceUtils(mContext).registerButtonLightOffTimeOut(resolver, mSettingsObserver);
            resolver.registerContentObserver(Settings.Secure.getUriFor(
    ...
    private void updateSettingsLocked() {
        ...
        PMSFactory.getInstance().createPowerManagerServiceUtils(mContext).updateButtonLightOffTimeOut(resolver);
    ...
    private void applyWakeLockFlagsOnAcquireLocked(WakeLock wakeLock, int uid) {
        if ((wakeLock.mFlags & PowerManager.ACQUIRE_CAUSES_WAKEUP) != 0
            ...
            if (DEBUG_WAKEUP) {
                ...
                Slog.d(TAG, "acquire the wakelock flags=0x" + Integer.toHexString(wakeLock.mFlags) +
                        "  packagename=" + opPackageName + " with wakeup flag");
            }
            //<-- Add power debug log Feature END
            //=============================================================================
    ...
    private boolean userActivityNoUpdateLocked(long eventTime, int event, int flags, int uid) {
        ...
        try {
            if (eventTime > mLastInteractivePowerHintTime) {
                if(TOUCH_TRIGGER_POWERHINT || event != PowerManager.USER_ACTIVITY_EVENT_BUTTON && event != PowerManager.USER_ACTIVITY_EVENT_TOUCH){
                    //=============================================================================
                    // add by sprd start
                    //NOTE: Bug #542086 Add power debug log Feature BEG-->
                    if (getDeBugPowerHintLog()) {
    ...
    private void shutdownOrRebootInternal(final @HaltMode int haltMode, final boolean confirm,
        ...
        Log.d(TAG,
                "Reboots the device , UID : " + Binder.getCallingUid() + " , PID : "
                        + Binder.getCallingPid() + " , haltMode = " + haltMode+" , reason  : "+reason);
        //<-- Add power debug log Feature END
        //=============================================================================
    ...
    private void powerHintInternal(int hintId, int data) {
        if(hintId == PowerManagerInternal.POWER_HINT_SUSTAINED_PERFORMANCE_MODE){
            AbsPowerManagerServiceUtils absPowerManagerServiceUtils = PMSFactory.getInstance().createPowerManagerServiceUtils(mContext);
            if(data == 1){
                if(!absPowerManagerServiceUtils.isThermalEnabled()){
                    absPowerManagerServiceUtils.thermalEnabled(true);
                }
            }else if (data == 0){
                if (absPowerManagerServiceUtils.isThermalEnabled()){
                    absPowerManagerServiceUtils.thermalEnabled(false);
                }
            }
        }
