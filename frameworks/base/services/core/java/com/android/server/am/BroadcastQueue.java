...
public final class BroadcastQueue {
    ...
    static final boolean DEBUG_BROADCAST_SPRD_TORTUOUS = false;
    ...
    final void processNextBroadcast(boolean fromMsg) {
        synchronized(mService) {
            ...
            if (!skip) {
                ...
                skip = skip || !mService.addListAndJudgeAllowLocked(info.activityInfo.applicationInfo.packageName, r.callerPackage,"send-broadcast");
            }
            ...
            if (Build.PERMISSIONS_REVIEW_REQUIRED && !skip) {
                if (!requestStartTargetPermissionsReviewIfNeededLocked(r,
                        info.activityInfo.packageName, UserHandle.getUserId(
                                info.activityInfo.applicationInfo.uid))) {
                    Slog.d(TAG,"requestStartTargetPermissionsReviewIfNeededLocked return false, skip");
                    skip = true;
                }
            }
            ...
            if (app != null && app.crashing) {
                skip = true;
                Slog.i(TAG_BROADCAST, "skip delivery of ordered [" + mQueueName + "] "
                        + r + " to app " + app + " because it is crashing");
            }
            if (!skip) {
            ...
            if (mService.avoidGmsProcessStart(r.callerApp, targetProcess)) {
                Slog.w(TAG, "Unable to launch app "+info.activityInfo.applicationInfo.packageName + " due to mem pressure");
                return;
            }
