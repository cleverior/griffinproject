...
public final class PinnerService extends SystemService {
    ...
    public void onStart() {
        ...
        String[] fileToReadAhead = mContext.getResources().getStringArray(com.android.internal.R.array.config_defaultPinnerServiceReadAheadFiles);
        for (int i = 0; i < fileToReadAhead.length; i++){
            boolean success = pinFileReadAhead(fileToReadAhead[i], 0, 0);
            if (success == true) {
                mPinnedFiles.add(fileToReadAhead[i]);
                Slog.i(TAG, "Pinned file = " + fileToReadAhead[i]);
            } else {
                Slog.e(TAG, "Failed to pin file = " + fileToReadAhead[i]);
            }
        }
    }

    private boolean pinFileReadAhead(String fileToPin, long offset, long length) {
