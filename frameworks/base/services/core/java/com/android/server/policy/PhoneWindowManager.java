...
public class PhoneWindowManager extends AbsPhoneWindowManager implements WindowManagerPolicy {
    ...
    protected boolean mKeyguardHidden;//modify for Bug#618149
    ...
    boolean mScreenshotChordVolumeUpKeyTriggered;
    boolean mScreenshotChordPowerKeyTriggered;
    long mScreenshotChordPowerKeyTime;
    ...
    private void interceptPowerKeyDown(KeyEvent event, boolean interactive) {
        ...
        if (gestureService != null) {
            /*SPRD:Resolve the conflict between SOS and camera.@{*/
        ...
        if (!mPowerKeyHandled) {
            ...
            /* SPRD: add log for debugging power key down @{*/
            Slog.v(TAG, "interceptPowerKeyDown, hungUp = " + hungUp
                + ", volumeDown = " + mScreenshotChordVolumeDownKeyTriggered
                + ", volumeUp = " + mScreenshotChordVolumeUpKeyTriggered
                + ", intercepted = " + gesturedServiceIntercepted);
            /*@}*/
        }
    }
    ...
    /*SPRD:add the feature of calling 112 SOS after press power key three times@{*/
    private long[] mHits = new long[3];
    public void touchToSos() {
           ...
           if (mHits[0] >= (SystemClock.uptimeMillis() - 2000)) {
                    ...
                    Log.d(TAG, "start Emergency Call");
                    mHits[0]=0l;
                    mHits[1]=0l;
          }
    }
    ...
    /*SPRD:check whether there is a simcard.@{*/
    private TelephonyManager mTelephonyManager;
    private boolean hasSimCard() {
        mTelephonyManager = TelephonyManager.from(mContext);
        int phoneCount = mTelephonyManager.getPhoneCount();
        if (1 == phoneCount) {
            return mTelephonyManager.hasIccCard(0);
        } else if(2 == phoneCount) {
            if (mTelephonyManager.hasIccCard(0) || mTelephonyManager.hasIccCard(1)) {
                return true;
            }
        }
        return false;
    ...
    private void handleLongPressOnHome(int deviceId) {
        ...
        switch (mLongPressOnHomeBehavior) {
            ...
            case LONG_PRESS_HOME_ASSIST:
                ...
                final int deviceIdx = deviceId;
                mHandler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        launchAssistActionX(null, deviceIdx);
                    }
                }, 500);
    ...
    private void readConfigurationDependentBehaviors() {
        ...
        /* SPRD: long press home show googlesearch feature bug505116 @{ */
    ...
    public void adjustWindowParamsLw(WindowManager.LayoutParams attrs) {
        ...
        /*SPRD: Add for bug 609263 avoid virtual key's background being changed when start a call.@{*/
    ...
    public static boolean isGestureKeyCode(int code){
        boolean res=false;
        switch(code){
    ...
    public static final File switchFile=new File("data/data/com.zyt.close_gesture_sttings/gesture_switch");
        ...
        if (mPendingCapsLockToggle && !KeyEvent.isMetaKey(keyCode) && !KeyEvent.isAltKey(keyCode)) {
            mPendingCapsLockToggle = false;
				...
				if (keyCode == KeyEvent.KEYCODE_F11) {
						TelecomManager telecomManager = getTelecommService();
            boolean disableFingerCall = Settings.Global.getInt(mContext.getContentResolver(),"allow_call", 0) == 0;
            if (!disableFingerCall && telecomManager != null && telecomManager.isRinging()) {
                 	telecomManager.acceptRingingCall();
                 	return 0;
            }else if(!disableFingerCall && telecomManager != null && telecomManager.isInCall()) {
    ...
    /*SPRD:Bug 593513 @{*/
    private void launchAssistActionX(String hint, int deviceId) {
        if (!isUserSetupComplete()) {
    ...
    private boolean layoutNavigationBar(int displayWidth, int displayHeight, int displayRotation,
        ...
        if (mNavigationBar != null) mNavigationBarOnBottom = isNavigationBarOnBottom(displayWidth, displayHeight);
        ...
        if (mStatusBar != null) {
		   ...
		   if (mForceStatusBar || mForceStatusBarFromKeyguard || mForceStatusBarTransparent
			...
			if (mForceStatusBar || mForceStatusBarFromKeyguard || mForceStatusBarTransparent) {		
                ...
                if (statusBarExpanded && mNavigationBar != null && mHasNavigationBar) {
    ...
    //zhangpeifu add for gesture
    android.util.Log.d("zyt.zhangpeifu.gesture5.1","event.isWakeKey()          "+event.isWakeKey()    +"          "+event);
    android.util.Log.d("zyt.zhangpeifu.gesture5.1",SystemProperties.get("ro.product.tp_ges").equals("true")+"          "+(event.getAction()==KeyEvent.ACTION_DOWN)+"    "+isGestureKeyCode(event.getKeyCode()));

    if(SystemProperties.get("ro.product.tp_ges").equals("true")
            &&!ActivityManager.isUserAMonkey()
                && (event.getAction()==KeyEvent.ACTION_DOWN)&&isGestureKeyCode(event.getKeyCode())){
        mKeyguardDelegate.doForGestureMediator(event.getKeyCode());

        android.util.Log.d("zyt.zhangpeifu.gesture5.1","  mKeyguardDelegate.doForGestureMediator  ");
    }
    //zhangpeifu add for gesture
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_POWER: {
                ...
                if (down) {
                    ...
                    Slog.d(TAG, "Receive Input KeyEvent of Powerkey down, interactive = " + interactive);
                    //<-- Add power debug log Feature END
                    //=============================================================================
                    ...
                    // add by sprd start
                    //NOTE: Bug #542086 Add power debug log Feature BEG-->
                    Slog.d(TAG, "Receive Input KeyEvent of Powerkey up, interactive = " + interactive);
                    //<-- Add power debug log Feature END
                    //=============================================================================
                    /*SPRD:add the feature of calling 112 SOS after press power key three times@{*/
                    if (SystemProperties.getBoolean("gesture.enable_sos_launch", false) && hasSimCard()) {
                        touchToSos();
                    }
                    /*@}*/
    ...
    public boolean hasNavigationBar() {
        return mHasNavigationBar || mDynamicNavigationBar;
    }
