...
class AlarmManagerService extends SystemService {
        ...
        */
        ...
        synchronized (mLock) {
            ...
            if (DEBUG_POWER_ISSUE) {
                ...
                long when = triggerAtTime;
                Object clientListener = null;
                ...
                try{
                    ...
                    formatDate = DateFormat.format("yyyy-MM-dd HH:mm:ss", when).toString();
                    if(operation != null){
                    ...
                    if(directReceiver != null){
                        Field field = directReceiver.getClass().getDeclaredField("mListener");
                        if(field != null){
                            field.setAccessible(true);
                            clientListener = field.get(directReceiver);
                ...
                Slog.d(TAG, "setImpl() action = "+action+", component = "+cn+", type = "+ type
                    + ", operation = "+operation
                    + ", tElapsed=" + triggerElapsed + ", maxElapsed=" + maxElapsed + ", interval=" + interval
                    + ", when = "+when+ ", triggerTime = "+ formatDate
                    + ", flags=0x" + Integer.toHexString(flags) + ", directReceiver=" + directReceiver
    ...
    void rescheduleKernelAlarmsLocked() {
        ...
        Slog.d(TAG, "rescheduleKernelAlarmsLocked -- mAlarmBatches.size() = " + mAlarmBatches.size());
        if (mAlarmBatches.size() > 0) {
            final Batch firstWakeup = findFirstWakeupBatchLocked();
            final Batch firstBatch = mAlarmBatches.get(0);
            Slog.d(TAG, "rescheduleKernelAlarmsLocked -- firstBatch = " + firstBatch + ", Alarm = " + firstBatch.get(0));
            // SPRD: Regular PowerOnOff Feature. start>
            ...
            if (firstWakeup != null && mNextWakeup != firstWakeup.start
               && !firstWakeup.hasExtraAlarm(POWER_ON_WAKEUP) && !firstWakeup.hasExtraAlarm(POWER_OFF_ALARM)) {
    ...
    private void setLocked(int type, long when) {
        Slog.d(TAG, "setLocked -- type = " + type + ", when = " + when);
        if (mNativeData != 0) {
            ...
            Slog.d(TAG, "setLocked set to native -- mNativeData = " + mNativeData + ", type = " + type
                    + ", alarmSeconds = " + alarmSeconds + ", alarmNanoseconds = " + alarmNanoseconds);
    ...
    boolean triggerAlarmsLocked(ArrayList<Alarm> triggerList, final long nowELAPSED,
        ...
        while (mAlarmBatches.size() > 0) {
            ...
            Slog.v(TAG, "[Current mAlarmBatches.size: " + mAlarmBatches.size() +  "]  [current batch0: " + batch + "]");
    ...
    private static class Alarm {
        ...
        public static String makeTag(PendingIntent pi, String tag, int type) {
            final String alarmString = type == ELAPSED_REALTIME_WAKEUP || type == RTC_WAKEUP || type == POWER_OFF_WAKEUP || type == POWER_OFF_ALARM
                    ? "*walarm*:" : "*alarm*:";
    ...
    private class AlarmThread extends Thread
    {
        ...
        public void run()
        {
            ...
            while (true)
            {
                ...
                // SPRD: Add Debug log. start>
                if (DEBUG_POWER_ISSUE) {
                    long whenCurrent = System.currentTimeMillis();
                    Slog.d(TAG, "waitForAlarm result:" + result + "currentTime=" + whenCurrent + ", elapsedRealtime=" + mLastWakeup);
                }
                // <end
                ...
                if (result != TIME_CHANGED_MASK) {
                    ...
                    synchronized (mLock) {
                        ...
                        if (0 == triggerList.size()) {
                            Slog.w(TAG, "triggerList.size = 0, reset the time tick event && reset the kernel alarm.");
                            mNextWakeup = 0;
                            mNextNonWakeup = 0;
                            // 1.Check the time tick alarm from the pending alarms.
                            boolean hasTimeTick = true;
                            if (mPendingNonWakeupAlarms.size() > 0) {
                                for (Alarm alarm : mPendingNonWakeupAlarms) {
                                    if (alarm.matches(mTimeTickSender, null)) {
                                        hasTimeTick = false;
                                        break;
                                    }
                                }
                            }
                            // 2.Check teh batch list
                            if (!hasTimeTick) {
                                for (Batch batch : mAlarmBatches) {
                                    Alarm alarm = batch.get(0);
                                    if (alarm != null && alarm.matches(mTimeTickSender, null)) {
                                        hasTimeTick = false;
                                        break;
                                    }
                                }
                            ...
                            if (!hasTimeTick) {
                                Slog.w(TAG, "Resend the time tick alarm.");
                                try {
                                    mTimeTickSender.send();
                                } catch (PendingIntent.CanceledException e) {
                                    Slog.w(TAG, "Resend the time tick alarm.", e);
                                }
                            } else {
                                Slog.w(TAG, "Reschedule the time tick alarm.");
                                rescheduleKernelAlarmsLocked();
                            }
                            continue;
                        }
                    ...
                    // SPRD: Bug604523.Add synchronizition for mAlarmBatches access.
                    synchronized (mLock) {
                      rescheduleKernelAlarmsLocked();
                    }
                }
