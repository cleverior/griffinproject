    ...
    private void handleShowingDetail(final QSTile.DetailAdapter adapter, int x, int y) {
        ...
        if (showingDetail) {
            ...
            if (toggleState == null) {
                ...
                /* SPRD: modify by BUG 620467 @{ */
                if (adapter.getTitle().equals(
                    mContext.getString(R.string.quick_settings_cellular_detail_title))
                    && ((isInCall() && netWorkClass < TelephonyManager.NETWORK_CLASS_3_G)
                    || isAirplaneModeOn())) {
                    mQsDetailHeader.setClickable(false);
                }
                /* @} */
