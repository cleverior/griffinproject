...
import com.zyt.close_gesture_sttings.GestureSystemCallBack;
import com.zyt.close_gesture_sttings.GestureDelegate;
 ...
 */
public class KeyguardViewMediator extends SystemUI implements GestureSystemCallBack{
    ...
    //zhangpeifu add for gesture
    public static final int MSG_DO_SCREEN_POWER_ON=100;
    public static final int RETURN_PHONE_WINDOW_HAND=103;
    public static final String PARA_TO_KEYGUARD_JUST_TURN="just_turn";

    public boolean isGestureCon=false;
    public Messenger   callGestureMsger ;
    public static final int CALL_JUST_TURN_ON_SCREEN_FROM_KEYGUARD=101;
    public static final long GESTURE_TIME_DELAYED= 200L;
    public static boolean needDismissBygesture=false;
    public int currentCode=-1;
	
	//tangyb
    public static final int PRESS_POWER=107;
    public static final String CALL_MEDIA_CHANG_MUSIC="music_chang";
    public boolean isMusicChange=false;
    
    public GestureDelegate mGestureDelegate;
    
 public void bindGestureCallBackService(Context c){
        if(!isGestureCon) {
            ComponentName cn = new ComponentName("com.zyt.close_gesture_sttings",
                                        "com.zyt.close_gesture_sttings.ServiceForKeyguardCallBack");
            android.util.Log.d("zyt.zhangpeifu.gesture5.1","bindGestureCallBackService  ");

            Intent intent = new Intent();
            intent.setComponent(cn);
            //new Intent("com.zyt.keyguardcallback")
            
            if(gestureCallServiceConnection==null){
            	gestureCallServiceConnection = new ServiceConnection() {
        @Override
        public void onServiceConnected(ComponentName name, IBinder service) {
            callGestureMsger= new Messenger(service);
            isGestureCon=true;
            android.util.Log.d("zyt.zhangpeifu.gesture5.1","onServiceConnected  ");
        }
        
        @Override
        public void onServiceDisconnected(ComponentName name) {
            gestureCallServiceConnection = null;
            isGestureCon=false;
        }
    };
        ...
        if(gestureCallServiceConnection!=null){    
            ...
            gestureCallServiceConnection, 
            Context.BIND_AUTO_CREATE,UserHandle.SYSTEM);
        }
        }
    }

    ServiceConnection gestureCallServiceConnection = new ServiceConnection() {
        @Override
        public void onServiceConnected(ComponentName name, IBinder service) {
            callGestureMsger= new Messenger(service);
            isGestureCon=true;
...
public Messenger replyGestureMsger =new Messenger(
        new Handler(){
            @Override
            public void handleMessage(Message msg){
                Bundle data=msg.getData();
                switch(msg.what){
                    case RETURN_PHONE_WINDOW_HAND:
                        long time  =  data.getLong(PARA_EVENT_TIME, -100L);
                        boolean justTurnOnScreen=msg.getData().getBoolean(PARA_TO_KEYGUARD_JUST_TURN, true);
                        needDismissBygesture=!justTurnOnScreen;
                        isMusicChange = msg.getData().getBoolean(CALL_MEDIA_CHANG_MUSIC, false);
                        android.util.Log.d("zyt.zhangpeifu.gesture5.1","needDismissBygesture     "+needDismissBygesture);
                        if(!justTurnOnScreen){
                           mHandler.sendEmptyMessage(DISMISS);
                            android.util.Log.d("zyt.zhangpeifu.gesture5.1","dismiss  ");
                        }
                        break;
                }
            }
 ...
 /*   public void callGesture( Bundle data){ 
        bindGestureCallBackService(mContext);
        Message gestureCallMsg = Message.obtain(null,CALL_JUST_TURN_ON_SCREEN_FROM_KEYGUARD);
        gestureCallMsg.replyTo = replyGestureMsger;
        gestureCallMsg.setData(data);
        try {
            android.util.Log.d("zyt.zhangpeifu.gesture5.1","callGesture    "+data);
            callGestureMsger.send(gestureCallMsg);
        } catch (RemoteException e) {
            android.util.Log.d("zyt.zhangpeifu.gesture5.1","callGesture  excetion  "+data+"      "+e);
            e.printStackTrace();
        }
    }
*/
...
public void callGesture2( Bundle data){ 
        if(mGestureDelegate==null){
        	mGestureDelegate=new GestureDelegate(mContext,this);
        	}
android.util.Log.d("zyt.zhangpeifu.gesture5.1","callGesture2  "+data);

       mGestureDelegate.doForGesture(data);
     ...
     @Override
public void systemuiCallBack(Bundle data){
	  long time  =  data.getLong(PARA_EVENT_TIME, -100L);
                        boolean justTurnOnScreen=data.getBoolean(PARA_TO_KEYGUARD_JUST_TURN, true);
                        needDismissBygesture=!justTurnOnScreen;
                        android.util.Log.d("zyt.zhangpeifu.gesture5.1","needDismissBygesture     "+needDismissBygesture);
                        if(!justTurnOnScreen){
                         // sendEmptyMessage(DISMISS);
                            mHandler.sendEmptyMessageDelayed(DISMISS,1000);
                            android.util.Log.d("zyt.zhangpeifu.gesture5.1","dismiss  ");
         }
...
static{// 
	KEY_ANIMA_MAP.put(KeyEvent.KEYCODE_DPAD_UP,		R.drawable.smart_key_up);
	KEY_ANIMA_MAP.put(KeyEvent.KEYCODE_DPAD_DOWN,	 	R.drawable.smart_key_down);
	KEY_ANIMA_MAP.put(KeyEvent.KEYCODE_DPAD_LEFT, 	 	R.drawable.smart_key_left);
	KEY_ANIMA_MAP.put(KeyEvent.KEYCODE_DPAD_RIGHT,	 	R.drawable.smart_key_right);
	KEY_ANIMA_MAP.put(KeyEvent.KEYCODE_U, 			R.drawable.smart_key_u);
	KEY_ANIMA_MAP.put(KeyEvent.KEYCODE_O, 			R.drawable.smart_key_o);
	KEY_ANIMA_MAP.put(KeyEvent.KEYCODE_W, 			R.drawable.smart_key_w);
	KEY_ANIMA_MAP.put(KeyEvent.KEYCODE_C, 			R.drawable.smart_key_c);
	KEY_ANIMA_MAP.put(KeyEvent.KEYCODE_E, 			R.drawable.smart_key_e);
	KEY_ANIMA_MAP.put(KeyEvent.KEYCODE_M, 			R.drawable.smart_key_m);
	KEY_ANIMA_MAP.put(KeyEvent.KEYCODE_S,			R.drawable.smart_key_s);
	KEY_ANIMA_MAP.put(KeyEvent.KEYCODE_W, 			R.drawable.smart_key_w);
	KEY_ANIMA_MAP.put(KeyEvent.KEYCODE_V, 			R.drawable.smart_key_v);
	KEY_ANIMA_MAP.put(KeyEvent.KEYCODE_Z, 			R.drawable.smart_key_z);
}

  //  Handler mBlackViewHandler;
    LinearLayout mBlackView;
    WindowManager.LayoutParams mBlackViewLp;
    ImageView animaImageView;
     AlphaAnimation gestureAnimation;
	boolean hasAddBlack=false;
       public static final int MSG_ADD_BLACKVIEW=104;
    public static final int MSG_REMOVE_BLACKVIEW=105;
   public static final int MSG_KEEP_BLACKMESSAGE=106;
       public static final String PARA_KEYCODE="key_code";
    public static final String PARA_EVENT_TIME="eventTime";
    public void comfirmAddBlackView(){
                	if(mBlackViewLp==null){
                	mBlackViewLp= new WindowManager.LayoutParams(
                    WindowManager.LayoutParams.MATCH_PARENT,
                    WindowManager.LayoutParams.MATCH_PARENT);
            mBlackViewLp.type = WindowManager.LayoutParams.TYPE_SECURE_SYSTEM_OVERLAY;
            mBlackViewLp.flags = WindowManager.LayoutParams.FLAG_FULLSCREEN
                    | WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE
                    | WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE
                    | WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN;
		}
			if(mBlackView==null){
				mBlackView=new LinearLayout(mContext);
				mBlackView.setBackgroundColor(0xff000000);
				mBlackView.setVisibility(View.GONE);
				}
		if(animaImageView==null){
		    animaImageView = new ImageView(mContext);  
		    animaImageView.setLayoutParams(new WindowManager.LayoutParams(WindowManager.LayoutParams.WRAP_CONTENT,WindowManager.LayoutParams.WRAP_CONTENT));  
			mBlackView.addView(animaImageView);
		}
	
					if(gestureAnimation==null){
			gestureAnimation = new AlphaAnimation(1, 0);
			gestureAnimation.setDuration(1500);
				     gestureAnimation.setFillAfter(true);

				gestureAnimation.setAnimationListener(new AnimationListener() {
		...
		public void onAnimationEnd(Animation animation) {
			// TODO Auto-generated method stub
			//gestureHandler.sendEmptyMessage(HIDE_GESTURE_ANIMA_IN_WINDOW);
			//animaFlag=false;
			//gestureHandler.sendEmptyMessage(REMOVE_GESTURE_ANIMA_WINDOW_IM);
			//mHandler.removeMessages(MSG_REMOVE_BLACKVIEW);
                        mHandler.sendEmptyMessageDelayed(MSG_REMOVE_BLACKVIEW,1000);
    ...
    public void onSystemReady() {
        ...
        synchronized (this) {
            ...
            if(SystemProperties.get("ro.product.tp_ges").equals("true")) {
               // bindGestureCallBackService(mContext);
               mGestureDelegate=new GestureDelegate(mContext,this);
                comfirmAddBlackView();
                android.util.Log.d("zyt.zhangpeifu.gesture5","systemReady  ");
            }
        }
        mIsPerUserLock = StorageManager.isFileEncryptedNativeOrEmulated();
        /* SPRD: telcel feature for 644278 @{ */
        boolean isShowDialog = false;
        if (mContext.getResources().getBoolean(R.bool.config_show_datause_dialog)) {
    ...
    protected void showPackageDialogIfNeeded(final Context context) {
        ...
        Dialog myAlertDialog = new AlertDialog.Builder(context)
                ...
                .setNegativeButton(R.string.dialog_messages_ok,
                        new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog,
                                    int which) {
    ...
    private SpannableString getClickableSpan() {
        View.OnClickListener l = new View.OnClickListener() {
            ...
            public void onClick(View v) {
                String telcelUrl = "http://pqta.telcel.com";
                Uri uri = Uri.parse(telcelUrl);
    ...
    //zhangpeifu add for gesture
    boolean shouldShowBlack=false;
    public void doForGestureMediator(int code) {
        shouldShowBlack=true;
        comfirmAddBlackView();
        android.util.Log.d("zyt.zhangpeifu.gesture5.1","systemUI doForGestureMediator    "+code);
        Message blackMsg=mHandler.obtainMessage(MSG_ADD_BLACKVIEW);
        Bundle data= new Bundle();
        data.putInt(PARA_KEYCODE, code);
        blackMsg.setData(data);
        mHandler.sendMessage(blackMsg);
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case ON_ACTIVITY_DRAWN:
                    ...
                    break;
                //zhangpeifu add for gesture
                case MSG_KEEP_BLACKMESSAGE:
                    if(hasAddBlack){
                        mBlackView.setVisibility(View.VISIBLE);	
                    }
                    break;

                case MSG_ADD_BLACKVIEW:
                    if(!hasAddBlack){
                        wm.addView(mBlackView,mBlackViewLp);
                        hasAddBlack=true;
                    }	
                    int animaCode=msg.getData().getInt(PARA_KEYCODE);
                    mBlackView.setVisibility(View.VISIBLE);	
						animaImageView.setImageResource(KEY_ANIMA_MAP.get(animaCode).intValue());
						animaImageView.setAnimation(gestureAnimation);
						gestureAnimation.reset();
						gestureAnimation.startNow();
					break;
					case MSG_REMOVE_BLACKVIEW:
						mBlackView.setVisibility(View.GONE);	
					break;
					//zhangpeifu add for gesture
            }
