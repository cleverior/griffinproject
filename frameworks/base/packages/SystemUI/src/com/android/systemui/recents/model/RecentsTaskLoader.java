...
class TaskResourceLoadQueue {
    ...
    /**
     * Retrieves the next task from the load queue, as well as whether we want
     * that task to be force reloaded.
     */
    Task nextTask() {
        return mQueue.poll();
    }
...
class BackgroundTaskLoader implements Runnable {
    ...
    /**
     * Constructor, creates a new loading thread that loads task resources in
     * the background
     */
    public BackgroundTaskLoader(TaskResourceLoadQueue loadQueue,
            TaskKeyLruCache<Drawable> iconCache,
            TaskKeyLruCache<ThumbnailData> thumbnailCache,
    ...
    void stop() {
        ...
        mCancelled = true;
        // If we are waiting for the load queue for more tasks, then we can just
        // reset the
    ...
    public void run() {
        while (true) {
            if (mCancelled) {
                // We have to unset the context here, since the background
                ...
                // If we've stopped the loader, then fall through to the above
                // logic to wait on
                // the load thread
                if (ssp != null) {
                    ...
                    if (t != null) {
                        ...
                        ThumbnailData cachedThumbnailData = mThumbnailCache
                                .get(t.key);

                        // Load the icon if it is stale or we haven't cached one
                        // yet
                        if (cachedIcon == null) {
                            cachedIcon = ssp.getBadgedTaskDescriptionIcon(
                                    t.taskDescription, t.key.userId,
                            ...
                            if (cachedIcon == null) {
                                ...
                                if (info != null) {
                                    ...
                                    cachedIcon = ssp.getBadgedActivityIcon(
                                            info, t.key.userId);
                                }
                            }
                            ...
                            // At this point, even if we can't load the icon, we
                            // will set the
                            // default icon.
                            mIconCache.put(t.key, cachedIcon);
                        }
                        // Load the thumbnail if it is stale or we haven't
                        // cached one yet
                        if (cachedThumbnailData == null) {
                            if (config.svelteLevel < RecentsConfiguration.SVELTE_DISABLE_LOADING) {
                                if (DEBUG)
                                    Log.d(TAG, "Loading thumbnail: " + t.key);
                                cachedThumbnailData = ssp
                                        .getTaskThumbnail(t.key.id);
                            }
                            ...
                            // When svelte, we trim the memory to just the
                            // visible thumbnails when
                            // leaving, so don't thrash the cache as the user
                            // scrolls (just load
                        ...
                        if (!mCancelled) {
                            ...
                            if (PhoneStatusBar.mSupportLockApp) {
                                ...
                                if (sharedPreference.contains(t.key
                                        .toStringKey())) {
                                    t.isLocked = true;
                                }
                            }
                            ...
                            mMainThreadHandler.post(new Runnable() {
                                @Override
                                public void run() {
                                    t.notifyTaskDataLoaded(
                                            newThumbnailData.thumbnail,
                                            newIcon,
                ...
                // If there are no other items in the list, then just wait until
                // something is added
...
public class RecentsTaskLoader {
    ...
    // This activity info LruCache is useful because it can be expensive to
    // retrieve ActivityInfos
    // for many tasks, which we use to get the activity labels and icons. Unlike
    // the other caches
    // below, this is per-package so we can't invalidate the items in the cache
    // based on the last
    // active time. Instead, we rely on the RecentsPackageMonitor to keep us
    // informed whenever a
    ...
    /**
     * Acquires the task resource data directly from the cache, loading if
     * necessary.
     */
    public void loadTaskData(Task t) {
        ...
        ThumbnailData thumbnailData = mThumbnailCache
                .getAndInvalidateIfModified(t.key);
        if (thumbnailData != null) {
            thumbnail = thumbnailData.thumbnail;
            thumbnailInfo = thumbnailData.thumbnailInfo;
        }

        // Grab the thumbnail/icon from the cache, if either don't exist, then
        // trigger a reload and
        ...
        t.notifyTaskDataLoaded(thumbnail == mDefaultThumbnail ? null
                : thumbnail, icon, thumbnailInfo);
    }
    ...
    /**
     * Handles signals from the system, trimming memory when requested to
    ...
    public void onTrimMemory(int level) {
        ...
        case ComponentCallbacks2.TRIM_MEMORY_COMPLETE:
            ...
            // The cache is small, only clear the label cache when we are
            // critical
    ...
    /**
     * Returns the cached task label if the task key is not expired, updating
     * the cache if it is.
     */
    String getAndUpdateActivityTitle(Task.TaskKey taskKey,
        ...
        // All short paths failed, load the label from the activity info and
        // cache it
        ...
        // If the activity info does not exist or fails to load, return an empty
        // label for now,
        // but do not cache it
        return "";
    }
    ...
    /**
     * Returns the cached task content description if the task key is not
     * expired, updating the cache if it is.
     */
    String getAndUpdateContentDescription(Task.TaskKey taskKey, Resources res) {
        SystemServicesProxy ssp = Recents.getSystemServices();

        // Return the cached content description if it exists
        String label = mContentDescriptionCache
                .getAndInvalidateIfModified(taskKey);
        if (label != null) {
            return label;
        }
        ...
        ActivityInfo activityInfo = getAndUpdateActivityInfo(taskKey);
        if (activityInfo != null) {
            label = ssp.getBadgedContentDescription(activityInfo,
                    taskKey.userId, res);
            mContentDescriptionCache.put(taskKey, label);
            return label;
        }
        // If the content description does not exist, return an empty label for
        // now, but do not
        // cache it
        return "";
    }
    ...
    /**
     * Returns the cached task icon if the task key is not expired, updating the
     * cache if it is.
     */
    Drawable getAndUpdateActivityIcon(Task.TaskKey taskKey,
    ...
    /**
     * Returns the cached thumbnail if the task key is not expired, updating the
     * cache if it is.
     */
    ...
    /**
     * Returns the task's primary color if possible, defaulting to the default
     * color if there is no specified primary color.
     */
    ...
    /**
     * Returns the activity info for the given task key, retrieving one from the
     * system if the task key is expired.
     */
    ActivityInfo getAndUpdateActivityInfo(Task.TaskKey taskKey) {
        ...
        if (activityInfo == null) {
            ...
            if (cn == null || activityInfo == null) {
                Log.e(TAG, "Unexpected null component name or activity info: "
                        + cn + ", " + activityInfo);
                return null;
            }
    ...
    public final void onBusEvent(PackagesChangedEvent event) {
        // Remove all the cached activity infos for this package. The other
        // caches do not need to
        // be pruned at this time, as the TaskKey expiration checks will flush
        // them next time their
        // cached contents are requested
        Map<ComponentName, ActivityInfo> activityInfoCache = mActivityInfoCache
                .snapshot();
