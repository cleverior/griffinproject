...
public class StorageNotification extends SystemUI {
    ...
    /*SPRD bug 645023:Show SDCard mounted notication in owner user only.{@*/
    private String publickSDMountedVolId = null;
    private int mCurrentUserId = UserHandle.myUserId();
    private final BroadcastReceiver mConfigrationReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            //
            if(intent == null){
            ...
            try {
                Log.d(TAG, "onReceive publickSDMountedVolId="+publickSDMountedVolId);
                final int currentId = intent.getIntExtra(Intent.EXTRA_USER_HANDLE, -1);
                Log.d(TAG, "onReceive currentId="+currentId+",UserHandle.USER_OWNER="+UserHandle.USER_OWNER);
                mCurrentUserId = currentId;
                if(mCurrentUserId != UserHandle.USER_OWNER){
                    mNotificationManager.cancelAsUser(publickSDMountedVolId, PUBLIC_ID, UserHandle.ALL);
                }else{
                    mNotificationManager.cancelAsUser(null, PUBLIC_ID, UserHandle.ALL);
    ...
    public void start() {
        ...
        /*SPRD bug 645023:Show SDCard mounted notication in owner user only.{@*/
        mContext.registerReceiver(mConfigrationReceiver, new IntentFilter(Intent.ACTION_USER_SWITCHED));
        /*@}*/
    ...
    protected void onConfigurationChanged(Configuration newConfig) {
        ...
        try {
            if(newConfig != null && newConfig.locale != mLocale){
                Log.d(TAG, "onConfigurationChanged mLocale="+mLocale+",newConfig.locale="+newConfig.locale);
                if(mNotificationManager == null || mStorageManager == null){
                    return;
                }
                mNotificationManager.cancelAsUser(null, PUBLIC_ID, UserHandle.ALL);
                mNotificationManager.cancelAsUser(null, PRIVATE_ID, UserHandle.ALL);
                mNotificationManager.cancelAsUser(null, DISK_ID, UserHandle.ALL);
                mNotificationManager.cancelAsUser(null, MOVE_ID, UserHandle.ALL);
    ...
    private Notification onVolumeMounted(VolumeInfo vol) {
        ...
        /*SPRD bug 645023:Show SDCard mounted notication in owner user only.{@*/
        if(mCurrentUserId != UserHandle.USER_OWNER){
            Log.w(TAG, "Only USER_OWNER can send SD mounted notification!");
            return null;
        }
        publickSDMountedVolId = vol.getId();
        /*@}*/
