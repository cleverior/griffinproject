...
public class RecentsActivity extends Activity implements ViewTreeObserver.OnPreDrawListener {
    ...
    /* SPRD: Bug 595075  quick cleaning. @{ */
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        boolean isSupportClearAllTasks = true;//RecentsActivityUtils.getInstance().isSupportClearAllTasks();
        if(isSupportClearAllTasks){
            ImageButton clearButton = (ImageButton) findViewById(R.id.clear_button);
            clearButton.setVisibility(View.VISIBLE);
            //RecentsActivityUtils.getInstance().init(this,mRecentsView, clearButton);
    ...
    private void reloadStackView() {
        ...
        mStack = loadPlan.getTaskStack();
        mRecentsView.onReload(mIsVisible, mStack.getTaskCount() == 0);
        mRecentsView.updateStack(mStack, true /* setStackViewTasks */);

        // Update the nav bar scrim, but defer the animation until the enter-window event
        boolean animateNavBarScrim = !launchState.launchedViaDockGesture;
        mScrimViews.updateNavBarScrim(animateNavBarScrim, mStack.getTaskCount() > 0, null);
        ...
        if (launchState.launchedFromApp) {
            Task launchTarget = mStack.getLaunchTarget();
            int launchTaskIndexInStack = launchTarget != null
                    ? mStack.indexOfStackTask(launchTarget)
                    : 0;
    ...
    public void onMultiWindowModeChanged(boolean isInMultiWindowMode) {
        ...
        mStack = loadPlan.getTaskStack();
        int numStackTasks = mStack.getStackTaskCount();
    ...
    /* SPRD: Bug 600421  recent double click. @{ */
    @Override
    protected void onResume() {
        // TODO Auto-generated method stub
        super.onResume();
        if(mIncompatibleAppOverlay != null){
            mIncompatibleAppOverlay.setVisibility(View.GONE);
        }
    }
    /* @} */
    ...
    public final void onBusEvent(AllTaskViewsDismissedEvent event) {
        SystemServicesProxy ssp = Recents.getSystemServices();
        /* SPRD: Bug 595075 new feature of quick cleaning. @{ */
        /* @} */
        //if (ssp.hasDockedTask() && isAnyNoLocked) {
        //} else {
            // Just go straight home (no animation necessary because there are no more task views)
            //dismissRecentsToHome(false /* animateTaskViews */);
        //}
        //SPRD bug 610951:Don't dismissRecentsToHome while there hasDockedTask and new feature of quick cleaning.
        ArrayList<Task> tasks = mStack.getStackTasks();
        boolean isAnyLocked = false;
        SharedPreferences sharedPreference = PreferenceManager
                .getDefaultSharedPreferences(getApplicationContext());
        for(Task task : tasks){
             if (sharedPreference.contains(task.key.toStringKey())) {
                 isAnyLocked = true;
                 break;
             }
        }
        ...
        if(ssp.hasDockedTask()){
            if(mRecentsView != null && !isAnyLocked){
