...
public class KeyguardBottomAreaView extends FrameLayout implements View.OnClickListener,
	...
	// add
	private static final int SHOW_VOICE_ASSIST = 1;
	// end
	// add
	private static final int SHOW_CAMERA = 2;
	private ResolveInfo resolved = null;
	// end
    ...
    private void updateCameraVisibility() {
		...
		// add
        resolved = resolveCameraIntent();
		if(resolved == null){
            new Thread(){
                @Override
                public void run(){
                    int i=1;
                    while((resolved = resolveCameraIntent()) == null){
						Log.e("lfok"," updateCameraVisibility while: " + (i++));
						try {
							Thread.sleep(1000);
					...
					Message msg = mHandler.obtainMessage(SHOW_CAMERA);
					mHandler.sendMessageDelayed(msg, 300);
					Log.e("lfok"," canLaunchCamera ok");
        ...
        boolean visible = !isCameraDisabled
                ...
                && mUserSetupComplete;
		if(resolved != null) {
			Log.d("lfok", "updateCameraVisibility >>> resolved != null");
		}else{
			Log.d("lfok", "updateCameraVisibility >>> resolved == null");
		}     
		Log.d("lfok", "mUserSetupComplete:" + mUserSetupComplete);
    ...
    private boolean canLaunchVoiceAssist() {
		if(!mAssistManager.canVoiceAssistBeLaunchedFromKeyguard()){
            new Thread(){
                ...
                public void run(){
                    int i=1;
                    while(!mAssistManager.canVoiceAssistBeLaunchedFromKeyguard()){
                        Log.e("lfok","while "+(i++));
						try {
							Thread.sleep(500);
						} catch (final InterruptedException e) {
					...
					Message msg = mHandler.obtainMessage(SHOW_VOICE_ASSIST);
					mHandler.sendMessage(msg);
					Log.e("lfok"," canLaunchVoiceAssist ok");
                    //mLeftAffordanceView.setImageDrawable(mContext.getDrawable(R.drawable.ic_phone_24dp));
                    //mLeftAffordanceView.setContentDescription(mContext.getString(R.string.accessibility_phone_button));
                    //updateLeftAffordanceIcon();
	...
	private Handler mHandler = new Handler(Looper.myLooper(), null, true /*async*/) {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case SHOW_VOICE_ASSIST:
                    Log.e("lfok"," mHandler ok");
					mLeftAffordanceView.setImageDrawable(mContext.getDrawable(R.drawable.ic_mic_26dp));
                    mLeftAffordanceView.setContentDescription(mContext.getString(R.string.accessibility_voice_assist_button));
                    break;
				case SHOW_CAMERA:
                    Log.e("lfok"," mHandler ok");
					mCameraImageView.setImageDrawable(mContext.getDrawable(R.drawable.ic_camera_alt_24dp));
                    mCameraImageView.setContentDescription(mContext.getString(R.string.camera_label));
