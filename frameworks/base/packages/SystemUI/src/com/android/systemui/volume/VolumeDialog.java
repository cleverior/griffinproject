...
public class VolumeDialog implements TunerService.Tunable {
    ...
    /* SPRD: bug 650599 show the volume slider during the process of dialing @{ */
    ...
    //systemui or key, ringer mode changed from vibrate to normal.
    private final String RINGER_MODE_CHANGED_VTON_FOR_NO_AUDIOPROFILE = "ringer_mode_changed_vton_for_no_audioprofile";
    ...
    private void updateRowsH() {
        ...
        for (VolumeRow row : mRows) {
            ...
            /* SPRD: bug 650599 show the volume slider during the process of dialing @{ */
            if(mTelecomManager.isInCall() && (row.stream == AudioManager.STREAM_VOICE_CALL) && mExpanded) {
                Log.v(TAG,"show volume slider in call");
    ...
    private void updateVolumeRowSliderH(VolumeRow row, boolean enable, int vlevel) {
        ...
        /*SPRD bug 620347: Only update slider while enable{@*/
        if(!enable){
            Log.d(TAG, "updateVolumeRowSliderH only active stream can update progress!!");
            return;
        }
        /*@}*/
        if (row.tracking) {
    ...
    private final class VolumeSeekBarChangeListener implements OnSeekBarChangeListener {
        ...
        public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
            ...
            if(mAudioManager.getRingerMode() == AudioManager.RINGER_MODE_VIBRATE
                    && progress != 0
                    && mRow.stream == AudioManager.STREAM_RING){
                Settings.System.putInt(mContext.getContentResolver(), RINGER_MODE_CHANGED_VTON_FOR_NO_AUDIOPROFILE, 1);
            }
        }
